...
class HFDatasetInferencePipeline(InferencePipeline):
    """
...
class PartitionedInferencePipeline(InferencePipeline):
    """
    General pipeline class for LLM inference using vLLM on a dataset's partition
    """
...
class ProviderHFDatasetInferencePipeline(HFDatasetInferencePipeline):
    """
    Abstract class for provider-based inference pipelines (vllm, togetherAI, etc)

    # TODO : Should not inherit InferencePipeline as it does not use vLLM
    """
    ...
    def __call__(
        ...
        if start_idx < len(inputs):
            output = self.run_inference(inputs[start_idx:], max_new_tokens=max_new_tokens, start_idx=start_idx, max_rows_to_process=max_rows_to_process)
    ...
    def run_inference(self, inputs: List, max_new_tokens: int = 128, start_idx: int = 0, max_rows_to_process: int = None, batch_size: int = 8):
        """
        ...
        if max_rows_to_process is not None:
            inputs = inputs[:max_rows_to_process]
        ...
        for batch_idx in tqdm(range(0, len(inputs), batch_size), desc="Processing batches"):
            ...
            while not success and retries <= MAX_RETRIES:
                try:
                    ...
                    results.extend(self.call_provider(request_data))
                    success = True
...
class TogetherAIHFDatasetInferencePipeline(ProviderHFDatasetInferencePipeline):
    """
...
class VLLMEndpointHFDatasetInferencePipeline(ProviderHFDatasetInferencePipeline):
    """
